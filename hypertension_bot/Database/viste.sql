CREATE OR REPLACE VIEW users.vw_avguser
 ...
 SELECT DISTINCT ON ("Measurement"."User_idUser") "Measurement"."User_idUser",
	...
	AVG(systolic)  FILTER (WHERE datetime::timestamp BETWEEN now()::timestamp - '1 month'::interval and now()::timestamp)::int as monthSystolic,
	AVG(diastolic) FILTER (WHERE datetime::timestamp BETWEEN now()::timestamp - '1 month'::interval and now()::timestamp)::int as monthDiastolic
